			...
			<table class="table table-hover">
				...
				<tbody>
					{{-- biến $todos được controller trả cho view
						...
						<tr>
							<td id="{{$SanPham->MaSP}}">{{$SanPham->MaSP}}</td>
							<td id="hoten-{{$SanPham->MaSP}}">{{$SanPham->TenSP}}</td>
							
							<td>
								<button data-url="{{ route('studentajax.show',$SanPham->MaSP) }}"​ type="button" data-target="#show" data-toggle="modal" class="btn btn-info btn-show">Detail</button>
								<button data-url="{{ route('studentajax.update',$SanPham->MaSP) }}"​ type="button" data-target="#edit" data-toggle="modal" class="btn btn-warning btn-edit">Edit</button>
								<button data-url="{{ route('studentajax.destroy',$SanPham->MaSP) }}"​ type="button" data-target="#delete" data-toggle="modal" class="btn btn-danger btn-delete">Delete</button>
							</td>
						</tr>
						@endforeach
					</tbody>
				...
				$('#form-add').submit(function(e){
					...
					$.ajax({
						...
						success: function(response) {
							...
							$('tbody').prepend('<tr><td id=""></td><td id="hoten-">'+response.data.hoten+'</td><td id="gioitinh-">'+response.data.gioitinh+'</td><td id="ngaysinh-">'+response.data.ngaysinh+'</td><td id="sdt-">'+response.data.sdt+'</td><td id="diachi-">'+response.data.diachi+'</td><td><button data-url="{{asset('')}}studentajax/"​ type="button" data-target="#show" data-toggle="modal" class="btn btn-info btn-show">Detail</button><button style="margin-left: 5px;" data-url="{{asset('')}}studentajax/"​ type="button" data-target="#edit" data-toggle="modal" class="btn btn-warning btn-edit">Edit</button><button style="margin-left: 5px;" data-url="{{asset('')}}studentajax/"​ type="button" data-target="#delete" data-toggle="modal" class="btn btn-danger btn-delete">Delete</button></td></tr>');
